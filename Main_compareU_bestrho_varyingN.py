...
J_result = np.zeros((5, len(N_list)))
div = 10
for N_idx in range(len(N_list)):
    ...
    print(rho_list)
    J_result[4, N_idx] = value

import matplotlib.pyplot as plt
plt.figure(1)
plt.plot(N_list, J_result[0,:], 'r-o')
plt.plot(N_list, J_result[1,:], 'b-^')
plt.plot(N_list, J_result[2,:], 'k-s')
plt.plot(N_list, J_result[3,:], 'g-d')
plt.plot(N_list, J_result[4,:], 'y-v')
plt.show()

import pandas as pd
varyingN_list = np.vstack((N_list, J_result))
df = pd.DataFrame(varyingN_list)
df.to_csv('CompareU_bestRho_varyingN.csv',index= False, header= False)